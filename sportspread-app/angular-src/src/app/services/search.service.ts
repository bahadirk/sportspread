...
export class SearchService {
  ...
  findOpponentTeam(search) {
    let headers = new Headers();
    this.loadToken();
    headers.append('Authorization', this.authToken);
    headers.append('Content-Type', 'application/json');
    return this.http.post('http://localhost:3000/users/opponentteam', search, {headers: headers})
      .map(res => res.json());
  }

  findTeamInstructor(search) {
    let headers = new Headers();
    this.loadToken();
    headers.append('Authorization', this.authToken);
    headers.append('Content-Type', 'application/json');
    return this.http.post('http://localhost:3000/users/teaminstructors', search, {headers: headers})
      .map(res => res.json());
  }
